...
import com.github.azurellf.todo.core.TodoIndexParameter;
...
public class TodoCommand {
    final private TodoItemService service;
    ...
    public int markAsDone(@CommandLine.Parameters(index = "0") final int index) {
        ...
        final Optional<TodoItem> doneItem = service.markTodoItemDone(TodoIndexParameter.of(index));
        if(doneItem.isEmpty()) {
            throw new CommandLine.ParameterException(spec.commandLine(), "unknown todo item index");
        }
        final TodoItem actual = doneItem.get();
    ...
    public int list(@CommandLine.Option(names = "--all") final boolean all) {
        final List<TodoItem> items = service.list(all);
        items.stream()
                .map(this::formatItem)
